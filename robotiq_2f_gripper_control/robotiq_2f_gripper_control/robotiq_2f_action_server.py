...
from .robotiq_2f_gripper_driver import (
    Robotiq2FingerGripperDriver,
    Robotiq2FingerSimulatedGripperDriver,
)
...
class CommandGripperActionServer(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "~stroke", 0.085
        )  # Default stroke is 85mm (Small C / 2 finger adaptive gripper model)
        self.declare_parameter("~joint_names", "finger_joint")
        self.declare_parameter("~sim", False)
        self.declare_parameter("~rate", 50)
        self.declare_parameter("~goal_detection_threshold", 0.0015)

        # Get Node parameters
        self._comport = (
            self.get_parameter("~comport").get_parameter_value().string_value
        )
        ...
        self._goal_threshold = (
            self.get_parameter("~goal_detection_threshold")
            .get_parameter_value()
            .double_value
        )
        ...
        if self._sim:  # Use simulated gripper
            self._driver = Robotiq2FingerSimulatedGripperDriver(
                stroke=self._stroke, joint_names=self._joint_names
            )
        else:  # Try to connect to a real gripper
            self._driver = Robotiq2FingerGripperDriver(
        ...
        self._action_server = ActionServer(
            self, CommandRobotiqGripper, "~/command", self.execute_cb
        )
        self._joint_trajectory_action_server = ActionServer(
            self,
            FollowJointTrajectory,
            "~/robotiq_controller/follow_joint_trajectory",
        ...
        self._gripper_state_publisher_timer = self.create_timer(
            1 / self._rate, self._update_driver
        )

    def _connection_timeout(self):
        self.get_logger().fatal(
            "Gripper on port {} seems not to respond".format(self._driver._comport)
        )
        self.shutdown()
    ...
    def execute_cb(self, goal_handle):
        ...
        self.get_logger().debug(
            (
                "New goal received Pos:{:.3f} Speed: {:.3f} Force: {:.3f} Force-Stop: {}"
            ).format(
        ...
        while (
            rclpy.utilities.ok() and self._processing_goal and not self._is_stalled
        ):  # While moving and not stalled provide feedback and check for result
            feedback.feedback = self._driver.get_current_gripper_status()
            goal_handle.publish_feedback(feedback)
            self.get_logger().debug(
                "Error = {:.5f} Requested position = {:.3f} Current position = {:.3f}".format(
                    abs(
                        feedback.feedback.requested_position
            ...
            if (
                feedback.feedback.fault_status != 0 and not self._is_stalled
            ):  # Check for errors
                self.get_logger().error(
                    "Fault status (gFLT) is: {:d}".format(
                        feedback.feedback.fault_status
                    )
                )
                ...
                return feedback
            if (
                abs(feedback.feedback.requested_position - feedback.feedback.position)
                < self._goal_threshold
                or feedback.feedback.obj_detected
        ...
        if not self._is_stalled:
            self.get_logger().debug(
                "Goal reached or object detected Pos: {:.3f} PosRequested: {:.3f} ObjectDetected: {}".format(
                    goal_command.position,
                    feedback.feedback.requested_position,
                    feedback.feedback.obj_detected,
                )
            )
            goal_handle.succeed()
        else:
            self.get_logger().error(
                "Goal aborted Pos: {:.3f} PosRequested: {:.3f} ObjectDetected: {}".format(
                    goal_command.position,
                    feedback.feedback.requested_position,
                    feedback.feedback.obj_detected,
                )
    ...
    def execute_joint_trajectory_cb(self, goal_handle):
        ...
        self.get_logger().info(
            "Trajectory received with {:d} points".format(len(goal.trajectory.points))
        )
        ...
        if len(goal_trajectory_point.positions) != 1:
            ...
            return result
        target_speed = (
            goal_trajectory_point.velocities[0]
            if len(goal_trajectory_point.velocities) > 0
            else 0.01
        )
        target_force = (
            goal_trajectory_point.effort[0]
            if len(goal_trajectory_point.effort) > 0
            else 0.1
        )
        ...
        while rclpy.utilities.ok() and self._processing_goal and not self._is_stalled:
            ...
            self.get_logger().debug(
                "Error : {:.3f} -- Actual: {:.3f} -- Desired: {:.3f}".format(
                    error,
            ...
            if current_status.fault_status != 0 and not self._is_stalled:
                ...
                result.error_code = -6
                result.error_string = (
                    "Gripper fault status (gFLT): " + current_status.fault_status
                )
                goal_handle.abort()
                return result
    ...
    def _execution_timeout(self):
        self.get_logger().error(
            "Achieving goal is taking too long, dropping current goal"
        )
        self._is_stalled = True
        self._processing_goal = False
